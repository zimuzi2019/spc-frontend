<template>
  <div id="content">
    ...
    <a-descriptions bordered style="margin: 5px">
      <a-descriptions-item label="总批次数">{{graphData.batchNum}}</a-descriptions-item>
      <a-descriptions-item label="子组总数">{{graphData.subgroupTotal}}</a-descriptions-item>
      <a-descriptions-item label="子组容量">{{graphData.subgroupCapacity}}</a-descriptions-item>
      <a-descriptions-item label="嵌套性">{{graphData.nestedness}}</a-descriptions-item>
    ...
    onMounted(() => {
      ...
      spcChartXBar.setOption({
        ...
        xAxis: {
          ...
          nameGap: 40,
          data: Array.from({length: graphData.value.batchNum}, (_, i) => i + 1),
        },
